...
//
//StreamDrawElementParser.prototype.LineCollectionSize = function(data, current)
//{
//   return data.indexOf(this.symb
...
StreamDrawSystemParser.prototype.DataScan = function(data, start, func)
{
   var current = start;
   var clength = 0;
   var scanned = 0;
   ...
   while(true)
   {
      ...
      if(cc == this.symbols.text)
         clength = 1 + this.eparse.MessageSize(data, current + 1);
...
{
   return this.symbols.text + this.eparse.CreateMessage(username, message);
};
...
StreamDrawSystemParser.prototype.CreateLines = function(type, page, lines, ignoredColors)
{
   ...
   if(type == "stroke") 
   {
      ...
      result = this.eparse.CreateStroke(lines, ignoredColors, this.symbols.cap + startChunk);
   }
   else if(type == "lines")
   {
      startChunk = this.symbols.lines + startChunk;
      result = this.eparse.CreateBatchLines(lines, ignoredColors);
   }
   else if(type == "rectangles")
   {
      startChunk = this.symbols.rectangles + startChunk;
      result = this.eparse.CreateBatchRects(lines, ignoredColors);
   }
   else
   {
...
{
   //Symbols are stored in the core becuase the data scanner needs it, along
   //with several inner-identifying things. Mostly: if ignore wasn't needed,
   //the symbols could go in the system and the core could be completely
   //identifier agnostic. Perhaps think about this.
